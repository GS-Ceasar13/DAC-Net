...
class TopKLoss(CrossentropyND):
    ...
    """
    def __init__(self, weight=None, ignore_index=-100, k=3):
        self.k = k
...
class WeightedCrossEntropyLossV2(torch.nn.Module):
    ...
    def forward(self, net_output, gt):
        ...
        class_weights = torch.cuda.FloatTensor([0.01,0.11,0.11,0.11,0.11,0.11,0.11,0.11,0.11,0.11])
        gt = gt.long()
        ...
        gt = gt.view(-1,)
        # print('*'*20)
        return F.cross_entropy(net_output, gt,weight=class_weights) # , weight=class_weights
